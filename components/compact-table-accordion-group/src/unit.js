...
describe('CompactTableAccordionGroup: OpenButton click triggers onChange', assert => {
...
describe('CompactTableAccordionGroup: Title triggers onChange when changeOnTitleClick is true', assert => {
  const handleChange = sinon.spy();
...
describe('CompactTableAccordionGroup: Title does not trigger onChange when changeOnTitleClick not set', assert => {
  const handleChange = sinon.spy();
...
describe('CompactTableAccordionGroup: Missing changeOnTitleClick property does not cause console error', assert => {
  const spy = sinon.spy(console, 'error');
  mount(
    <Component title="Test" expanded="Expanded">
      Test
    </Component>,
  );
